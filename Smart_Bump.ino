  ...
  if (sonar.ping_cm() == 0){   
    Serial.println("Nothing");
      ...
      for(int i = 0; i<10; i++){              
        ...
        for ( int i = 0; i < 5 ; i++){          //цикл сделан зарание, далее скорости будут превращены в одно среднеарифметичесое значение(еще не не написано)
        double speedCar = arrayRasDist[i]/arrayRasTime[i] ;     // получение скорости по формуле U = S/t
        arraySpeed[i] = abs(speedCar);  
        Serial.print("Speed number (");         // запись полученной скорости в массив
        Serial.print(i);                  
        Serial.print("):");
